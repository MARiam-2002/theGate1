...
itemsSchema.query.pagination = function (page) {
  page = !page || page < 1 || isNaN(page) ? 1 : page;
  const limit = 16;
  const skip = limit * (page - 1);
  return this.skip(skip).limit(limit);
};
itemsSchema.query.customSelect = function (fields) {
...
itemsSchema.methods.inStock = function (requiredQuantity) {
  return this.availableItems >= requiredQuantity ? true : false;
};

const itemsModel =
  mongoose.models.itemsModel || model("Item", itemsSchema);
